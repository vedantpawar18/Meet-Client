...
export default function UploadPage() {
  ...
  async function submit(e) {
    e.preventDefault();
    if (!file) return setStatus({ type: "error", text: "Select XML file" });
    setStatus({ type: "loading" });
    const res = await dispatch(uploadParcelsXml(file));
    if (res.error)
      setStatus({
        type: "error",
        text: res.payload?.message || "Upload failed",
      });
    else
      setStatus({
        type: "success",
        text: `Created ${res.payload.created || 0} parcels`,
      });
  }
  return (
    <Paper sx={{ p: 2 }}>
