...
export default function ParcelsPage() {
  ...
  async function doApprove(id) {
    await dispatch(approveInsurance(id));
  }
  async function doDelete(id) {
    if (!window.confirm("Delete this parcel? This action cannot be undone."))
      return;
    const res = await dispatch(deleteParcel(id));
    if (res.error) {
      dispatch(
        showNotification({
          message: res.payload?.message || "Delete failed",
          severity: "error",
        })
      );
      return;
  ...
  async function doDeleteAll() {
    if (
      !window.confirm(
        "Delete ALL parcels? This will remove all parcels permanently. Continue?"
      )
    )
      return;
    ...
    if (res.error) {
      dispatch(
        showNotification({
          message: res.payload?.message || "Delete all failed",
          severity: "error",
        })
      );
      return;
  ...
  const filteredList = React.useMemo(() => {
    ...
    return arr.filter((p) => {
      if (dept) {
        ...
        if (!(String(id) === String(dept) || String(name) === String(dept)))
          return false;
      }
      if (insurance) {
    ...
    arr.sort((a, b) => {
      try {
        if (
          sortBy === "trackingId" ||
          sortBy === "destination" ||
          sortBy === "assignedDepartment"
        ) {
          const aa =
            (sortBy === "assignedDepartment"
              ? a.assignedDepartment?.name || ""
              : a[sortBy] || ""
            )?.toString() || "";
          const bb =
            (sortBy === "assignedDepartment"
              ? b.assignedDepartment?.name || ""
              : b[sortBy] || ""
            )?.toString() || "";
    ...
    <Box>
      ...
      >
        ...
        <TextField
          ...
          sx={{ width: 180 }}
        />
        {(fromDate || toDate || specificDate) && (
          <IconButton onClick={handleClearDates} title="Clear dates">
            <ClearIcon />
          </IconButton>
        )}
        ...
        <TableContainer component={Paper}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell
                  sortDirection={sortBy === "trackingId" ? sortDir : false}
                >
                  <TableSortLabel
                    active={sortBy === "trackingId"}
                    direction={sortBy === "trackingId" ? sortDir : "asc"}
                ...
                <TableCell
                  sortDirection={sortBy === "weightKg" ? sortDir : false}
                >
                  <TableSortLabel
                    active={sortBy === "weightKg"}
                    direction={sortBy === "weightKg" ? sortDir : "asc"}
                ...
                <TableCell
                  sortDirection={sortBy === "valueEur" ? sortDir : false}
                >
                  <TableSortLabel
                    active={sortBy === "valueEur"}
                    direction={sortBy === "valueEur" ? sortDir : "asc"}
                    onClick={() => handleSort("valueEur")}
                  >
                    Value
                  </TableSortLabel>
                </TableCell>
                <TableCell
                  sortDirection={sortBy === "destination" ? sortDir : false}
                >
                  <TableSortLabel
                    active={sortBy === "destination"}
                    direction={sortBy === "destination" ? sortDir : "asc"}
                ...
                <TableCell
                  sortDirection={
                    sortBy === "assignedDepartment" ? sortDir : false
                  }
                >
                  <TableSortLabel
                    active={sortBy === "assignedDepartment"}
                    direction={
                      sortBy === "assignedDepartment" ? sortDir : "asc"
                    }
            ...
            <TableBody>
              {pagedList.map((p) => (
                <TableRow
                  ...
                  sx={{
                    backgroundColor:
                      p.insuranceApproval?.status === "pending"
                        ? "rgba(244, 67, 54, 0.08)"
                        : "inherit",
                    "&:hover": {
                      backgroundColor:
                ...
                >
                  <TableCell>
                    <Link to={`/parcels/${p._id}`}>{p.trackingId}</Link>
                  </TableCell>
                  ...
                  <TableCell>
                    {p.assignedDepartment?.name || p.assignedDepartment || "-"}
                  </TableCell>
                  <TableCell>
                    {p.insuranceApproval?.status === "pending" &&
                    user?.role === "insurance" ? (
                      <Button onClick={() => doApprove(p._id)}>Approve</Button>
                    ) : p.insuranceApproval?.status === "pending" ? (
                      "Yet to be approved"
                    ) : (
                      p.insuranceApproval?.status || "-"
                    )}
                  </TableCell>
                  {user?.role === "admin" && (
                    <TableCell>
