...
export default function RulesPage() {
  ...
  useEffect(() => {
    ...
    const aggregate = (list || []).find((r) =>
      Array.isArray(r.config?.buckets)
    );
    if (aggregate) {
      console.debug(
        "RulesPage: found aggregate rule",
        aggregate._id,
        aggregate.name
      );
      for (const b of aggregate.config.buckets || []) {
        ...
        const found = (departments || []).find(
          (d) => String(d._id) === String(id) || d.name === id
        );
        if (found) {
          map[found._id] =
    ...
    try {
      ...
      if (aggregate) {
        ...
        const idx = buckets.findIndex(
          (b) =>
            b.departmentId === dept._id ||
            b.departmentId === dept.name ||
            b.department === dept._id
        );
        if (idx !== -1) {
          // replace
        ...
        const payload = {
          name: aggregate.name || "weight-based-routing",
          type: aggregate.type || "weight",
          priority: aggregate.priority ?? 10,
          config: { buckets },
        };
        ...
        if (res.error)
          dispatch(
            showNotification({
              message: res.payload?.message || "Update failed",
              severity: "error",
            })
          );
        else {
        ...
        const payload = {
          ...
          type: "weight",
          priority: 10,
          config: { buckets: [{ departmentId: dept._id, maxKg: num }] },
        };
        const res = await dispatch(createRule(payload));
        if (res.error)
          dispatch(
            showNotification({
              message: res.payload?.message || "Create failed",
              severity: "error",
            })
          );
        else {
      ...
      {user?.role === "admin" && (
        <Paper sx={{ p: 2, mb: 2 }}>
          <Typography variant="h6">
            Department rules (set Max KG per department)
          </Typography>
          ...
          <Paper key={key} sx={{ p: 2, mb: 1 }}>
              ...
              {user?.role === "admin" && (
                <Box>
                  <Button
                    color="error"
                    onClick={async () => {
                      // delete rule/bucket if exists
                      if (!window.confirm("Delete rule for this department?"))
                        return;
                      ...
                      if (aggregate) {
                        const buckets = (aggregate.config.buckets || []).filter(
                          (b) =>
                            String(b.departmentId) !== String(d._id) &&
                            b.departmentId !== d.name
                        );
                        const payload = {
                          name: aggregate.name || "weight-based-routing",
                          type: aggregate.type || "weight",
                        ...
                        const res = await dispatch(
                          updateRule({ id: aggregate._id, payload })
                        );
                        if (res.error) {
                          dispatch(
                            showNotification({
                      ...
                      if (!existing) {
                        dispatch(
                          showNotification({
                            message: "No rule to delete",
                            severity: "warning",
                          })
                        );
                        return;
                      }
                      const res = await dispatch(deleteRule(existing._id));
                      if (res.error)
                        dispatch(
                          showNotification({
                            message: res.payload?.message || "Failed",
            ...
            <Box sx={{ mt: 1, display: "flex", gap: 2, alignItems: "center" }}>
              <TextField
                ...
                value={maxByDept[key] ?? ""}
                onChange={(e) => {
                  setMaxByDept((prev) => ({ ...prev, [key]: e.target.value }));
                }}
                sx={{ width: 160 }}
              />
            </Box>
