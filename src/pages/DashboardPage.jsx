...
export default function DashboardPage() {
  ...
  const pendingInsurance = parcels.filter(
    (p) => p.insuranceApproval?.status === "pending"
  ).length;
    ...
    for (const p of parcels) {
      ...
      if (id && map[id] !== undefined) map[id] += 1;
        ...
        list.push({
          id,
          name:
            typeof p.assignedDepartment === "string"
              ? p.assignedDepartment
              : p.assignedDepartment.name || id,
        });
      }
    }
  ...
  const pieData = useMemo(() => {
    const arr = deptList.map((d, idx) => ({
      name: d.name,
      value: deptCountsMap[d.id] || 0,
      color: COLORS[idx % COLORS.length],
    }));
    // add unassigned slice
    if (unassigned > 0)
      arr.unshift({ name: "Unassigned", value: unassigned, color: "#9e9e9e" });
  ...
  const barData = useMemo(() => {
    return deptList
      .map((d) => ({ name: d.name, value: deptCountsMap[d.id] || 0 }))
    ...
    <Box>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          ...
          <Button variant="contained" onClick={() => navigate("/parcels")}>
            Open Parcels
          </Button>
        </Box>
      </Box>
